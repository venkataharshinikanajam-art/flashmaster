    ...
    <div className="rounded-xl border border-slate-800 bg-slate-900/50">
      ...
      >
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="text-slate-500 text-sm">{expanded ? "Hide" : "Show"}</span>
        </div>
      </div>
...
function CreateForm({ onCreated }) {
  ...
  // Set difficulty for a topic. Clicking the same level does nothing extra.
  const setTopicDifficulty = (topic, level) => {
                ...
                >
                  ...
                  {["easy", "medium", "hard"].map((d) => (
                    <button
                      key={d}
                      type="button"
                      onClick={() => setTopicDifficulty(topic, d)}
